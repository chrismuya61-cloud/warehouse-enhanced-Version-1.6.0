...
/**
 * Get Warehouse Option (Cached)
 */
function get_warehouse_option($name){
    $CI = & get_instance();
    static $wh_options_cache = [];

    if(isset($wh_options_cache[$name])){
        return $wh_options_cache[$name];
    }
    ...
    $wh_options_cache[$name] = $val;
    return $val;
}

/**
 * Check if reference exists in a specific table
 */
function is_reference_in_table($field, $table, $value) {
    $CI = & get_instance();
...
/**
 * Reformat Currency for Warehouse Logic
 */
...
// ... (Keep existing logic for handling tags, statuses etc from original file) ...
/**
